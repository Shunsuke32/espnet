...
from espnet3.systems.lid.task import LIDTask
...
class Speech2Language:
    ...
    def __init__(
        ...
        if not self.dtype.is_floating_point:
            raise ValueError(f"dtype must be floating point: {dtype}")
    ...
    def _load_languages(lang2utt: str | Path) -> list[str]:
        ...
        if not languages:
            raise ValueError(f"No languages found in lang2utt: {lang2utt}")
        if len(languages) != len(set(languages)):
            raise ValueError(f"Duplicate languages found in lang2utt: {lang2utt}")
        return languages
    ...
    def __call__(self, speech):
        ...
        Raises:
            ...
            RuntimeError: If a prediction is outside the language mapping.
        """
        ...
        if any(index < 0 or index >= len(self.languages) for index in indices):
            raise RuntimeError(
                "LID model returned a class index outside the lang2utt mapping"
            )
