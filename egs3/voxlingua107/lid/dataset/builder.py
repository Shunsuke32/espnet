...
class VoxLingua107Builder(DatasetBuilder):
    ...
    def prepare_source(self, source_dir: str | Path | None = None, **_kwargs) -> None:
        """Require an already downloaded and extracted VoxLingua107 corpus."""
        source_root = resolve_source_root(source_dir)
        raise FileNotFoundError(
            "VoxLingua107 is not prepared. Download and extract the corpus under "
            f"{source_root}; training audio must be under <lang>/ and development "
            "audio under dev/<lang>/."
        )
